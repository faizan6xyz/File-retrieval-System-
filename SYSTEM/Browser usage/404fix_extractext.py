...
class MCPClient:
        ...
        with self._lock:
            ...
            for attempt in range(3):
                ...
                if resp.status_code == 404:
                    print(f"[DEBUG] 404 received for '{method}' (attempt {attempt+1}). Body: {resp.text[:500]}")
                    print(f"[DEBUG] Session ID that was rejected: {self._session_id}")
                    ...
                    if attempt < 2:
                        ...
                        raise RuntimeError(f"Session expired after {attempt+1} retries")
                ...
                if "mcp-session-id" in resp.headers:
                    ...
                    if new_id != self._session_id:
                        print(f"[DEBUG] Session ID updated by '{method}' response: "
                              f"{self._session_id} -> {new_id}")
                    self._session_id = new_id
                    
                return self._parse_sse(resp.text)
            
            raise RuntimeError(f"Failed to complete {method} after all retries")
    ...
    def _handshake(self):
        ...
        print(f"[DEBUG] initialize response headers: {dict(resp.headers)}")
        print(f"[DEBUG] initialize response body: {resp.text[:500]}")

        if "mcp-session-id" in resp.headers:
            self._session_id = resp.headers["mcp-session-id"]
        print(f"[DEBUG] Session ID after initialize: {self._session_id}")

        notif_resp = self._do_post({"jsonrpc": "2.0", "method": "notifications/initialized"})
        print(f"[DEBUG] notifications/initialized response headers: {dict(notif_resp.headers)}")
        print(f"[DEBUG] notifications/initialized response body: {notif_resp.text[:500]}")

        if "mcp-session-id" in notif_resp.headers:
            new_id = notif_resp.headers["mcp-session-id"]
            if new_id != self._session_id:
                print(f"[DEBUG] *** SESSION ID CHANGED during notifications/initialized: "
                      f"{self._session_id} -> {new_id} ***")
                self._session_id = new_id
...
def structure_snapshot_with_llm(snapshot: str, url: str, mcp_client: MCPClient) -> str:
    # Validate session before expensive LLM processing
    try:
        mcp_client._rpc("ping", {})
    except Exception:
        print("[WARN] Session stale, refreshing before LLM processing...")
        mcp_client._session_id = None
        mcp_client._handshake()
