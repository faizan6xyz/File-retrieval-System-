"""
Query tool for the file_history.jsonl log produced by file_watcher.py.

The tricky part: a single logical file can be renamed and/or moved
multiple times, so its "history" isn't one fixed path - it's a chain
of paths over time. This walks the log backwards from a given current
path to reconstruct every previous path that file has had, then
collects every event (created/modified/renamed/moved/deleted) tied to
any path in that chain, sorted chronologically.

Usage:
    python query_history.py --log file_history.jsonl --path "C:/Watched/report_final.docx"
    python query_history.py --log file_history.jsonl --recent 20
    python query_history.py --log file_history.jsonl --action deleted
"""
